...
class _ExampleData {
  final TextEditingController bjn = TextEditingController();
...
class _MeaningData {
  final List<_DefinitionData> definitions = [_DefinitionData()];
  void dispose() {
...
class _DerivativeData {
  ...
  final TextEditingController syllables = TextEditingController();
  final List<_DerivDefData> definitions = [_DerivDefData()];
  void dispose() {
    word.dispose();
    syllables.dispose();
    for (final d in definitions) {
...
class SubmitWordView extends StatefulWidget {
  const SubmitWordView({super.key});

  @override
  State<SubmitWordView> createState() => _SubmitWordViewState();
}

class _SubmitWordViewState extends State<SubmitWordView> {
  final _formKey = GlobalKey<FormState>();
  final _wordCtrl = TextEditingController();
  final _syllablesCtrl = TextEditingController();
  String? _alphabet;
  final List<_MeaningData> _meanings = [_MeaningData()];
  final List<_DerivativeData> _derivatives = [];
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await repo.submit(WordSubmitRequest(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(title: 'Usulkan Kata', isClipped: isClipped),
      body: Stack(
        children: [
                      ...
                      children: [
                        TextFormField(
                          ...
                          decoration: const InputDecoration(
                            labelText: 'Kata *',
                            hintText: 'Masukkan kata dalam bahasa Banjar',
                          ),
                          validator: (v) =>
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(labelText: 'Abjad *'),
  ...
  List<Widget> _buildMeaningsSection(bool isDark) {
      ...
      widgets.add(
        _sectionCard(
          ...
          children: [
            ...
            OutlinedButton.icon(
              onPressed: () => setState(() {
                meaning.definitions.add(_DefinitionData());
              }),
    ...
    widgets.add(
      OutlinedButton.icon(
        onPressed: () => setState(() => _meanings.add(_MeaningData())),
        icon: const Icon(Icons.add, size: 16),
          ...
          child: Column(
            ...
            children: [
              ...
              TextButton.icon(
                ...
                label: const Text('Tambah Contoh', style: TextStyle(fontSize: 13)),
              ),
            ],
          ),
        ),
